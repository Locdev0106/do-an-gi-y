...
function Adidas({ searchQuery, addToCart, updateQuantity, removeFromCart }) {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const isAdidasInName = name.includes("adidas"); // Lọc sản phẩm có tên chứa "Adidas"
    const isAdidasBrand = brand === "adidas"; // Lọc theo brand nếu muốn
    ...
    return (
      ...
      isWithinPriceRange && 
      hasSelectedSize && 
      (isAdidasInName || isAdidasBrand) // Hiển thị sản phẩm nếu tên hoặc brand là "Adidas"
    );
  });
