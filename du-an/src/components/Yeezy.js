...
import YeezyCard from "./YeezyCard";
import "./ProductList.css";
import { useSelector } from "react-redux"; // Import useSelector
import "./Yeezy.css";

function Yeezy({ searchQuery, addToCart, updateQuantity, removeFromCart }) {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const isYeezyInName = name.includes("yeezy");  
    const isYeezyBrand = brand === "yeezy";  
    ...
    return (
      ...
      isWithinPriceRange && 
      hasSelectedSize && 
      (isYeezyInName || isYeezyBrand) // Hiển thị sản phẩm nếu tên hoặc brand là "Yeezy"
    );
  });
